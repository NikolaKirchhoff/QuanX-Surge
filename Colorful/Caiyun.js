...
根据平台添加如下配置
(1).Quantumult X
添加重写：https://raw.githubusercontent.com/NikolaKirchhoff/QuanX-Surge/master/Colorful/Caiyun.qxrewrite
(2).Surge
添加模块：https://raw.githubusercontent.com/NikolaKirchhoff/QuanX-Surge/master/Colorful/Caiyun.sgmodule
or https://raw.githubusercontent.com/NikolaKirchhoff/QuanX-Surge/master/Colorful/Caiyun-NoCron.sgmodule
2️⃣ 打开手机设置 > 隐私 > 定位服务
(1) 打开定位服务
...
if (typeof $request !== "undefined") {
  ...
  const res =
    url.match(/lon=(\d+(?:\.\d+)?)&lat=(\d+(?:\.\d+)?)/) ||
    url.match(/\/(\d+(?:\.\d+)?),(\d+(?:\.\d+)?)\/weather/);
  if (res === null) {
  ...
  $.write(res[2], "#latitude");
  $.write(res[1], "#longitude");
...
async function query() {
  ...
  const weather = await $.http.get({
    url,
    headers: {
      "User-Agent": "ColorfulCloudsPro/5.0.10 (iPhone; iOS 16.0; Scale/3.00)",
    },
  })
    .then((resp) => {
